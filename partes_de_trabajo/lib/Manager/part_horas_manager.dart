...
import '../main.dart';
import 'manager_new_part.dart';

class managerPartHoras extends StatefulWidget {
  String work_id;
  String work_name;
  String work_hours;
  managerPartHoras (this.work_id, this.work_name, this.work_hours);

  @override
  _managerPartHorasState createState() => _managerPartHorasState();
}

class _managerPartHorasState extends State<managerPartHoras> {
  ...
  Future<bool> _onBackPressed() {
    if(company_id != 'no')  Navigator.of(context).pushNamedAndRemoveUntil('/manager', (Route<dynamic> route) => false, arguments: ScreenArguments(company_id, 'key', 'value'));
  }
        ...
        appBar: new AppBar(
          ...
          actions: [ IconButton(
              icon: Icon(Icons.add, color: Colors.white,), onPressed: (){
                 Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => managerNewkPart(widget.work_id, manager_id, widget.work_name)));
              ...
              child:  SizedBox( // Horizontal ListView
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    manager_id == list[index]['createdfor'].toString() ? InkWell(onTap: (){ deleteCustomPart(context, list[index]['data'].toString(), list[index]['id'].toString());},
        ...
        if (inputDataPart.text == data_part){
          deletePartId(id_part)
            ...
            Scaffold.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).translate('deleted'),),));
              Navigator.push(context, MaterialPageRoute(builder: (context) => managerPartHoras(widget.work_id, widget.work_name, value.toString())));
          });
          return;
        } else { Scaffold.of(context).showSnackBar(SnackBar(content: Text(AppLocalizations.of(context).translate('dont_deleted_date'),),),); }
      },
    );
    ...
    AlertDialog alert = AlertDialog(
      ...
      content: Container(
        child: SingleChildScrollView(
          child: Column( children: [ Text(AppLocalizations.of(context).translate('insert_date') + ' ( ' + data_part  + ' ) ' + AppLocalizations.of(context).translate('for_delete'),),
